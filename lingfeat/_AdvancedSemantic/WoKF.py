...
def get_probability_lists(token_list, dir_path):
    ...
    lda_list = [
        gensim.models.ldamodel.LdaModel.load(
            dir_path + "/_AdvancedSemantic/model/enwiki" + str(n_topic)
        )
...
def retrieve(token_list, dir_path):
    ...
    result = {
        f"WRich{n_topic_list_for_naming[i]}_S": feature
        for i, feature in enumerate(richness_list)
    }

    for i,feature in enumerate(clarity_list):
        result[f"WClar{n_topic_list_for_naming[i]}_S"] = feature
    for i,feature in enumerate(noise_list):
        result[f"WNois{n_topic_list_for_naming[i]}_S"] = feature
    for i,feature in enumerate(n_topics_list):
        result[f"WTopc{n_topic_list_for_naming[i]}_S"] = feature
